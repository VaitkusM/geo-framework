...
ToricPatch::ToricPatch(size_t num_sides, size_t depth) : NPatch("NOTHING.tp", num_sides), d_(depth), normalized(false)
{
  initDomain();
  ...
  tdu_ = box_max[0] - box_min[0];
  tdv_ = box_max[1] - box_min[1];
...
ToricPatch::initDomain()
{
  ...
  for(size_t i = 0; i < n_; ++i) {
    ...
    auto g = std::gcd(size_t(std::abs(r[0])), size_t(std::abs(r[1])));
    r /= g;
...
ToricPatch::getBlendFunctions(double u, double v, std::map<Index, double>& values) const
{
  ...
  for (const auto& [id, cp] : net_) {
    ...
    for (size_t i = 0; i < n_; ++i) {
      ...
      auto lab = sideDistance(id[0], id[1], i);
      B *= std::pow(luv, size_t(lab));
    }
    values[id] = B;
    sum += B;
  }
